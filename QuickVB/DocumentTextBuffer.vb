...
Public Class DocumentTextBuffer
    ...
    Public ReadOnly Property Document As Document
        Get
            Return App.TheWorkspace.CurrentSolution.GetDocument(DocumentId)
        End Get
    End Property
    ...
    Public Sub Colorize(Optional diagnostics As IReadOnlyList(Of Diagnostic) = Nothing, Optional cancel As CancellationToken = Nothing)
        ...
        Dim text = Document.GetTextAsync(cancel).Result
        Dim rootNode = Document.GetSyntaxRootAsync(cancel).Result
        If rootNode Is Nothing Then Exit Sub

        Dim spans = Classifier.GetClassifiedSpansAsync(Document, rootNode.FullSpan).Result
        ...
        For Each span In spans
            ...
            If Document.GetSyntaxRootAsync.Result.Language = LanguageNames.VisualBasic Then
            ...
            Dim colorSpan As New ColorSpan With
            {
                .Start = spanStart.Character,
        ...
        If diagnostics IsNot Nothing Then
            For Each diagnostic In diagnostics.Where(Function(d) d.Location.SourceTree Is Document.GetSyntaxTreeAsync.Result)
                ...
                Dim cs = New ColorSpan
                cs.Start = startCol
                cs.Length = endCol - startCol
                cs.IsSticky = True
                cs.ForegroundColor = ConsoleColor.Gray
